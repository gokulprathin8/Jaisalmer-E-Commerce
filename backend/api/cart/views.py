...
def productById(request, id):
    ...
    if request.method == 'GET':
        ...
        product_data = JSONParser().parse(request)
        product_serializer = seralizers.ProductSerializer(
...
def productsRatingById(request, uid, pid):
    ...
    if request.method == 'GET':
        try:
            productRating = models.ProductRating.objects.get(
                product=pid, user=uid)
            ...
            if ((row['product_volume'] is not None) and int(mid[0])):
                try:
                    row['product_volume'] = eval(
                        "Volume(cubic_meter = row['product_volume']).{unit}".format(unit=units[counter]))
                    counter += 1
            ...
            if ((row['product_area'] is not None) and int(mid[1])):
                try:
                    row['product_area'] = eval(
                        "Area(sq_m = row['product_area']).{unit}".format(unit=units[counter]))
                    counter += 1
                except AttributeError:
                    return JsonResponse({
                        'message': 'Invalid Units Given'
            ...
            if ((row['product_mass'] is not None) and int(mid[2])):
                try:
                    row['product_mass'] = eval(
                        "Mass(g = row['product_mass']).{unit}".format(unit=units[counter]))
                    counter += 1
                except AttributeError:
                    return JsonResponse({
                        'message': 'Invalid Units Given'
            ...
            if ((row['product_weight'] is not None) and int(mid[3])):
                try:
                    row['product_weight'] = eval(
                        "Weight(g = row['product_weight']).{unit}".format(unit=units[counter]))
                    counter += 1
                except AttributeError:
                    return JsonResponse({
                        'message': 'Invalid Units Given'
            ...
            if ((row['product_time'] is not None) and int(mid[4])):
                try:
                    row['product_time'] = eval(
                        "Time(s = row['product_time']).{unit}".format(unit=units[counter]))
                    counter += 1
                except AttributeError:
                    return JsonResponse({
                        'message': 'Invalid Units Given'
