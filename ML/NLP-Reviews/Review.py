...
# Droping Columns
col_drop = ['reviewerID', 'reviewerName', 'helpful',
            'unixReviewTime', 'reviewTime', 'summary']

dataset.drop(columns=col_drop, axis=1, inplace=True)

# Renaming The Column Names

dataset.rename(columns={'asin': 'ProductID',
                        'reviewText': 'Review', }, inplace=True)
...
for i in range(0, len(sentences)):
    ...
    review = review.lower()
    review = review.split()
    review = [lematizer.lemmatize(word)for word in review if word not in set(
